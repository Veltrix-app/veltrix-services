...
import { submitEnterpriseIntake } from "@/lib/commercial/lead-intake";

type EnterpriseIntakeBody = {
...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json().catch(() => null)) as EnterpriseIntakeBody | null;
    ...
    const requirementSummary = sanitize(body?.requirementSummary);
    const securityRequirements = sanitize(body?.securityRequirements);
    const billingRequirements = sanitize(body?.billingRequirements);
    const onboardingRequirements = sanitize(body?.onboardingRequirements);
    ...
    if (requirementSummary.length < 12) {
      return NextResponse.json(
        { ok: false, error: "Describe what makes this an enterprise review request." },
        { status: 400 }
      );
    }

    const result = await submitEnterpriseIntake({
      requesterName,
      requesterEmail,
      companyName,
      teamSize,
    ...
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Enterprise intake submission failed.",
      },
      { status: 500 }
    );
  }
