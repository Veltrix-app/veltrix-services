...
export function mapProfile(row: Record<string, unknown>): UserProfile {
  ...
  return {
    ...
    walletVerified,
    xp: Number(reputation.total_xp ?? row.xp ?? 0),
    activeXp: Number(reputation.active_xp ?? reputation.total_xp ?? row.xp ?? 0),
    level: Number(reputation.level ?? row.level ?? 1),
    streak: Number(reputation.streak ?? row.streak ?? 0),
